...
const BibleForm = () => {
  ...
  const loadData = async () => {
    try {
      const data = await BibleApi.getVerses(form.versionId, form.bookId, form.chapter);
  ...
  const handleChange = (e) => {
    ...
    setForm({ ...form, [name]: name === "chapter" || name === "verse" ? Number(value) : value });
  };

  const validate = () => {
    const tmpErr = [];

    if (!form.chapter || form.chapter < 1) tmpErr.push("장을 정확히 입력해주세요.");

    if (!form.verse || form.verse < 1) tmpErr.push("절을 정확히 입력해주세요.");

    if (!form.text) tmpErr.push("구절을 입력해주세요.");
      ...
      <div className="container px-4 mt-5 mb-5">
        <div className="row gx-4 justify-content-center">
          <div className="col-lg-9 mb-2">
            <h4>
              <i className="bi bi-pencil text-primary fs-4"></i> 성경등록
            </h4>
          </div>
            ...
            <div className="card mb-4">
              ...
              <div className="card-body">
                <form className="row g-3" onSubmit={handleSubmit}>
                  <BibleSchSelect form={form} setForm={setForm}></BibleSchSelect>
