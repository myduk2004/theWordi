...
const BibleSearch = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const filteredBooks = data.books.filter(
        (b) => b.versionId === defaultVersionId && b.testament === defaultTestament
      );
  ...
  const handleChange = (e) => {
    ...
    if (name === "versionId") {
      ...
      const filterBook = allBook.filter(
        (b) => b.versionId === value && b.testament === defaultTestament
      );
      setBook(filterBook);
      ...
      const filterBook = allBook.filter(
        (b) => b.versionId === searchForm.versionId && b.testament === value
      );
      setBook(filterBook);
  ...
  return (
    <>
      ...
      <div
        className="col-md-12 collapse overflow-auto"
        style={{ maxHeight: "300px" }}
        id="collapseExample"
      >
        <div className="card card-body position-relative pt-5">
          {/* 부트스트랩의 toggle의움직임 사용을 위해 toggleBtn사용 */}
          <button
            type="button"
